...
interface BarcodeScanningPrintingProps {
  barcodeScanningPrintingData: {
    title: string;
    description: string;
    mainImageUrl: string;
    keyFeatures: string[];
...
export function BarcodeScanningPrintingClient({ barcodeScanningPrintingData }: BarcodeScanningPrintingProps) {
  const steps = barcodeScanningPrintingData?.howItWorksSteps || [];
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-16">
        <div className="relative mb-12 overflow-hidden">
          <ScrollAnimationWrapper>
            ...
            >
              {barcodeScanningPrintingData?.title || 'Barcode Scanning & Printing'}
            </motion.h1>
          </ScrollAnimationWrapper>
          <motion.div
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center mb-20">
          <ScrollAnimationWrapper>
            <motion.div className="space-y-6" variants={fadeInUp}>
              <p className="text-lg leading-relaxed">
                {barcodeScanningPrintingData?.description || 'Description not available'}
              </p>
              <h2 className="text-2xl font-semibold">Key Features</h2>
              <ul className="list-none pl-5 space-y-3">
                {(barcodeScanningPrintingData?.keyFeatures || []).map((feature, index) => (
                  <motion.li 
                    key={index}
          ...
          <ScrollAnimationWrapper>
            <div className="h-96 relative">
              <Image
                src={barcodeScanningPrintingData?.mainImageUrl || '/placeholder-image.jpg'}
                alt="Barcode Scanning & Printing"
                layout="fill"
                objectFit="cover"
